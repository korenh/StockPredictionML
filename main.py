...
class StockPrediction():
    ...
    def future(self):
        ...
        model_inupts = total_dataset[len(
            total_dataset) - len(test_data) - self.prediction_days:].values
        model_inupts = model_inupts.reshape(-1, 1)
        model_inupts = self.scaler.transform(model_inupts)

        real_data = [
            model_inupts[len(model_inupts) + 1 - self.prediction_days:len(model_inupts+1), 0]]
    ...
    def show(self):
        ...
        for x in range(self.prediction_days, len(model_inupts)):
            x_test.append(model_inupts[x-self.prediction_days:x, 0])
